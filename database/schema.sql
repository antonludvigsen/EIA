...
/* EIA database schema er kodet direkte fra oprettelsen, da vores DCD afspejler præcist de værdier skal skal indgå i hver domæneobjekt. 

Derudover kan vi afspejle PK og FK gennem vores ER-diagram på forhånd. 

Vi har desuden valgt at angive all værdier uden æ, ø, å så der ikke opstår friktion overall */

/* Bruger */
CREATE TABLE Bruger (
    brugerID INT PRIMARY KEY IDENTITY(1,1),
    navn NVARCHAR(100) NOT NULL,
    oprettetDato DATETIME DEFAULT GETDATE() /* opretter automatisk tidspunkt */
);

/* hardcoded til en bruger (da der ikke er krav om mulighed for flere oprettede brugere) */
INSERT INTO Bruger (navn, oprettetDato)
VALUES ('EIA Bruger', '2026-04-02');
...
CREATE TABLE Adresse (
    ...
    etage NVARCHAR(20) NULL, /* kan være kl., st. 1, 2, 3 osv. */
    doer NVARCHAR(20) NULL, /* th., tv. */
    postnummer NVARCHAR(10) NOT NULL,
    bynavn NVARCHAR(100) NOT NULL
);

/* Ejendomsdata */
/* når vi anvender decimaler bruges float. Man kunne normalt set også have brugt DOUBLE, men det er ikke en mulighed i T-SQL */
...
CREATE TABLE InvesteringsParametre (
    ...
    udlejningMaanederAarligt INT NULL,
    maanedligLeje FLOAT,
...
CREATE TABLE SimuleringResultat (
    ...
    aktiverOverTid NVARCHAR(MAX),
    likviditetsholdningOverTid NVARCHAR(MAX),
...
Herfra starter det oprindelige script med CREATE TABLE... */
